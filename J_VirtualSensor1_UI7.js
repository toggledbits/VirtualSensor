...
var VirtualSensor = (function(api) {
    ...
    function isBinarySensor( devobj ) {
        return devobj.device_type == "urn:schemas-micasaverde-com:device:DoorSensor:1" ||
            devobj.device_type == "urn:schemas-micasaverde-com:device:MotionSensor:1" ||
            devobj.device_type == "urn:schemas-micasaverde-com:device:SmokeSensor:1";
    }
    ...
    function handleRowChange( ev ) {
        ...
        var service, variable, matchstr;
        if ( isNaN( device ) ) {
            service = variable = matchstr = "";
        } else {
            var vs = jQuery( 'select.varmenu', row ).val() || "";
            variable = vs.replace( /^[^\/]+\//, "" );
            service = vs.replace( /\/.*$/, "" );
            if ( isBinarySensor( api.getDeviceObject( vsdevice ) ) ) {
                matchstr = jQuery( 'input#matchstring', row ).val() || "";
            } else {
                matchstr = "";
            }
        }
            ...
            'onSuccess' : function() {
                ...
                api.setDeviceStateVariablePersistent( vsdevice, serviceId, "SourceServiceId", service || "",
                {
                    'onSuccess' : function() {
                        ...
                        {
                            'onSuccess' : function() {
                                api.setDeviceStateVariablePersistent( vsdevice, serviceId, "MatchString", matchstr,
								{
									'onSuccess': function() {
										jQuery.ajax({
...
console.log("restart request finished");
										});
									}
								});
                            }
    ...
    function updateCurrentValues() {
        jQuery( 'div#vs-content div.vssensor' ).each( function() {
            ...
            if ( ! isNaN( sourcedevice ) ) {
                ...
                if ( "" !== variable ) {
                    str += '<br/>' + hval( String( api.getDeviceStateVariable( vs, serviceId, "PreviousRawValue" ) || "" ) );
                    str += " &rarr; ";
                    str += hval( String( api.getDeviceStateVariable( vs, serviceId, "RawValue" ) || "" ) );
                }
                col.html( str );
            } else {
                col.text( "n/a" );
    ...
    function redrawChildren() {
        ...
        for ( ix=0; ix<(devices || []).length; ix++ ) {
            ...
            if ( v.id_parent == myDevice ) {
                ...
                col.append( dm );
                dm.on( 'change.vsensor', handleRowChange );

                if ( isBinarySensor( v ) ) {
                    /* Binary sensors can have match string */
                    col.append( '<label>starts with <input id="matchstring" class="form-control">(optional)</label>' );
                    jQuery( 'input#matchstring', col ).val( api.getDeviceStateVariable( v.id, serviceId, "MatchString" ) || "" )
