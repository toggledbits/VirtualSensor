...
via the alternate web API, below.
...
configured. That is, if the value set is outside the range of function's
...
This action is really intended to be used only when the sensor is *Disabled* 
(i.e. the function generator is not running/used).

## Alternate Web API for Control ##

VirtualSensor (>=1.3) supports an alternate method of setting/resetting tripped state
arming/disarming, or setting a sensor's value. By
